(()=>{
'use strict';
const VERSION='login-success-warp-v5-mobile-parity';
...
style.textContent=`
#shooLoginWarpOverlay{--ally-left:max(20px,8vw);--scene-y:42%;--muzzle-x:calc(max(20px,8vw) + 118px);--shot-distance:calc(50vw - max(20px,8vw) - 118px);position:fixed;inset:0;width:100vw;height:100dvh;z-index:2147483650;pointer-events:none;overflow:hidden;background:transparent;color:#e8fbff;isolation:isolate;transform:translateZ(0)}
#shooLoginWarpOverlay::before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 50% 50%,rgba(56,189,248,.1),transparent 35%),repeating-linear-gradient(90deg,transparent 0 9vw,rgba(103,232,249,.035) 9vw calc(9vw + 1px));opacity:0;transition:opacity .35s ease}
#shooLoginWarpOverlay.portal::before{opacity:1}
.shooAllyCraft{position:absolute;left:-190px;top:var(--scene-y);width:126px;height:76px;transform:translate3d(0,-50%,0) scale(.82);transform-origin:center;opacity:0;filter:drop-shadow(0 0 18px rgba(56,189,248,.85));transition:left .62s cubic-bezier(.18,.82,.2,1),opacity .2s ease,transform .62s cubic-bezier(.18,.82,.2,1);will-change:left,transform,opacity}
#shooLoginWarpOverlay.approach .shooAllyCraft{left:var(--ally-left);opacity:1;transform:translate3d(0,-50%,0) scale(1)}
...
.shooAllyLabel{position:absolute;left:-12px;right:-12px;top:-18px;color:#67e8f9;font:1000 8px/1 ui-monospace,monospace;letter-spacing:.15em;text-align:center;text-shadow:0 0 8px #0369a1;white-space:nowrap}
.shooWarpCharge{position:absolute;left:var(--muzzle-x);top:var(--scene-y);width:32px;height:32px;margin:-16px;border-radius:50%;background:radial-gradient(circle,#fff 0 18%,#67e8f9 30%,#2563eb 62%,transparent 70%);box-shadow:0 0 18px #fff,0 0 42px #38bdf8;opacity:0;transform:scale(.25);will-change:transform,opacity}
#shooLoginWarpOverlay.charge .shooWarpCharge{opacity:1;animation:shooWarpChargePulse .45s ease-in-out infinite alternate}
.shooWarpShot{position:absolute;left:var(--muzzle-x);top:var(--scene-y);width:40px;height:40px;margin:-20px;border-radius:50%;opacity:0;background:radial-gradient(circle,#fff 0 16%,#a5f3fc 27%,#38bdf8 48%,#4f46e5 68%,transparent 72%);box-shadow:0 0 20px #fff,0 0 54px #38bdf8;will-change:transform,opacity}
...
.shooWarpCover{position:absolute;left:50%;top:50%;width:20px;height:20px;margin:-10px;border-radius:50%;background:#e0fbff;box-shadow:0 0 60px #67e8f9,0 0 150px #2563eb;opacity:0;transform:scale(0);will-change:transform,opacity}
#shooLoginWarpOverlay.cover .shooWarpCover{opacity:1;animation:shooWarpCover .62s cubic-bezier(.18,.75,.15,1) forwards}
.shooWarpStatus{position:absolute;left:50%;bottom:max(10%,calc(env(safe-area-inset-bottom) + 18px));transform:translateX(-50%);max-width:calc(100vw - 28px);box-sizing:border-box;padding:9px 14px;border:1px solid rgba(103,232,249,.38);border-radius:999px;background:rgba(2,6,23,.86);color:#dffbff;font:900 10px/1.2 ui-monospace,monospace;letter-spacing:.14em;text-align:center;white-space:nowrap;opacity:0;transition:opacity .25s ease;box-shadow:0 0 22px rgba(56,189,248,.25)}
...
@keyframes shooWarpShotFlight{0%{transform:translate3d(0,0,0) scale(.65);opacity:1}100%{transform:translate3d(var(--shot-distance),calc(50vh - var(--scene-y)),0) scale(1.15);opacity:1}}
...
@keyframes shooWarpCover{0%{transform:scale(0)}100%{transform:scale(140)}}
@keyframes shooWarpExit{to{opacity:0}}
@media(max-width:780px),(pointer:coarse){
 #shooLoginWarpOverlay{--ally-left:max(14px,5vw);--scene-y:38%;--muzzle-x:calc(max(14px,5vw) + 108px);--shot-distance:calc(50vw - max(14px,5vw) - 108px);height:100dvh;min-height:-webkit-fill-available}
 .shooAllyCraft{left:-170px;transform:translate3d(0,-50%,0) scale(.88)}
 #shooLoginWarpOverlay.approach .shooAllyCraft{transform:translate3d(0,-50%,0) scale(.88)}
 .shooWarpPortal{width:72px;height:72px;margin:-36px}
 .shooWarpStatus{bottom:max(8%,calc(env(safe-area-inset-bottom) + 14px));padding:8px 11px;font-size:9px;letter-spacing:.1em}
 .shooWarpShot::after{width:78px}
}
@media(max-width:390px){
 #shooLoginWarpOverlay{--scene-y:36%;--muzzle-x:calc(max(12px,4vw) + 100px);--ally-left:max(12px,4vw);--shot-distance:calc(50vw - max(12px,4vw) - 100px)}
 .shooAllyCraft{transform:translate3d(0,-50%,0) scale(.8)}
 #shooLoginWarpOverlay.approach .shooAllyCraft{transform:translate3d(0,-50%,0) scale(.8)}
 .shooWarpStatus{white-space:normal;width:calc(100vw - 24px)}
}
@media(orientation:landscape) and (max-height:520px){
 #shooLoginWarpOverlay{--scene-y:45%}
 .shooWarpStatus{bottom:max(5%,calc(env(safe-area-inset-bottom) + 8px))}
}
...
function tone(type,start,end,duration,volume,delay=0){
 ...
 try{const osc=ctx.createOscillator(),gain=ctx.createGain();osc.type=type;osc.frequency.setValueAtTime(start,now);osc.frequency.exponentialRampToValueAtTime(Math.max(20,end),now+duration);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(volume,now+.015);gain.gain.exponentialRampToValueAtTime(.0001,now+duration);osc.connect(gain).connect(ctx.destination);osc.start(now);osc.stop(now+duration+.03)}catch{}
}
...
function loginScreenVisible(){const el=document.getElementById('loginScreen');if(!el||el.classList.contains('hidden'))return false;const s=getComputedStyle(el);return s.display!=='none'&&s.visibility!=='hidden'&&el.getClientRects().length>0}
...
function runWarp(goHome=forceHome){
 ...
 try{window.showAppNotice?.({title:'ALLY LINK ESTABLISHED',message:'ログイン認証に成功しました。味方機がホーム座標へ転送します。',type:'success',duration:4200})}catch{}
...
function isSuccessText(text){const t=String(text||'').trim();return /^(Googleでログインしました。?|ログインしました。?)$/.test(t)}
