(()=>{
'use strict';
const VERSION='login-success-warp-v4-force-home';
...
function isEligibleHome(target){return String(target)==='home'&&!running&&Date.now()<=armedUntil&&loginScreenVisible()&&validAccount()}
...
function arm(){armedUntil=Date.now()+30000;primeAudio()}
...
function inspectSuccess(el){if(!isSuccessText(el?.textContent)||running||!loginScreenVisible()||!validAccount())return;arm();runWarp(forceHome)}
...
function bind(){
 ...
 document.querySelectorAll('#loginScreen button').forEach(button=>{if(button.dataset.shooWarpBound==='1')return;button.dataset.shooWarpBound='1';button.addEventListener('pointerdown',()=>armFromButton(button),{passive:true})});
