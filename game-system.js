...
// ui.js
root.UISystem={toast(message,ms=1800){const el=document.createElement('div');el.textContent=message;const mobile=matchMedia('(max-width: 700px), (pointer: coarse)').matches;Object.assign(el.style,{position:'fixed',left:'50%',bottom:mobile?'max(150px, calc(env(safe-area-inset-bottom) + 150px))':'24px',transform:'translateX(-50%)',zIndex:'999999',maxWidth:mobile?'calc(100vw - 24px)':'none',padding:'10px 16px',borderRadius:'999px',background:'#111827',color:'#fff',fontWeight:'800',textAlign:'center'});document.body.appendChild(el);setTimeout(()=>el.remove(),ms)},confirm(message){return window.confirm(message)}};
