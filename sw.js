const CACHE_NAME = "shooking-ii-v94";
const SW_BUILD = "94";

const GAME_SCRIPTS = [
  ["ui-patch.js",6],["app-notice.js",8],["release-current.js",13],["firebase-config.js",2],
...
self.addEventListener("activate", event => {
  event.waitUntil((async()=>{
    ...
    // Do not navigate or reload clients here.
  })());
});

function escapeRegExp(value){return value.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}
function removeScript(html,filename){
  return html.replace(new RegExp(`<script[^>]+src=["'][^"']*${escapeRegExp(filename)}[^"']*["'][^>]*><\\/script>`,"gi"),"");
}
...
function appendStyle(html,filename,version){
  html=html.replace(new RegExp(`<link[^>]+href=["'][^"']*${escapeRegExp(filename)}[^"']*["'][^>]*>`,"gi"),"");
